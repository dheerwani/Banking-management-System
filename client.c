...
// Receive message from the server in form of packets and readin it
...
// Sending messages to the server as in the form of the packets
...
int main(int argc,char **argv)
{
    ...
    printf("\n\n****************Welcome to the Dheer's Online Bank*****************\n\n");
    ...
    while(1) {
        ...
        if(strncmp(msgToServer, "exit", 4) == 0) {
                shutdown(sock_fd, SHUT_WR);
            break;
        }// Checking if the USer wants to continue after the Credit and debit Transaction
